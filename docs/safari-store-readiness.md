...
| --- | --- | --- | --- |
| S1 | **2.4.5(i) App Sandbox** — Mac App Store apps must be sandboxed | PASS | `safari-app/App/BookmarkAI.entitlements` and `Extension/BookmarkAIExtension.entitlements` contain exactly `com.apple.security.app-sandbox=true` (least privilege — the converter's `files.user-selected.read-only` was dropped; the appex needs no network entitlement because Safari performs the extension's requests). `project.yml` also sets `ENABLE_APP_SANDBOX: YES` + `ENABLE_HARDENED_RUNTIME: YES`. Built Debug bundle: `codesign -d --entitlements -` on app and appex → `app-sandbox` (+ `get-task-allow`, Debug only). Pinned by `safari-app/safari-app.test.ts`. |
...
| P9 | Chrome/Firefox outputs untouched | **FIXED** → PASS | No change to `wxt.config.ts`, `lib/`, `entrypoints/`; `scripts/generate-icons.mjs` was refactored to export its renderer and the 15 toolbar PNGs re-generated byte-identical (sha256). One trap surfaced and was closed: Tailwind v4 scans every non-ignored file under `apps/extension` for class-like tokens, so the new Swift/YAML/script files (a comment saying "container app") added a dead `.container` rule to the popup CSS and moved the popup chunk hashes on ALL browsers. `assets/tailwind.css` now carries `@source not` for `safari-app/` and the five new script files (only those — `scripts/generate-icons.mjs` already contributes a dead `.rounded` to the shipped CSS and had to stay). Verified: `chrome-mv3/assets/popup-C3j3pSDT.css` and `chunks/popup-B60519-I.js` are byte-identical to the 05:41 `bookmark-aiextension-0.1.2-chrome.zip`, file lists and manifests identical for Chrome and Firefox. The one remaining byte in `background.js` (a minified identifier inside Clerk's `safe-buffer` module, `xP()`→`Xue()`) reproduces from the untouched HEAD tree with this work stashed — dependency/environment drift since that zip, not this change. |
...
PASS 15 · FIXED 12 · FAIL 1 (support URL, needs a deploy) · NEEDS-TARA 9 · RISK 3 (4.8, screenshots, policy text until deployed) · INFO 4.
...
Screenshots (QA evidence — contain Tara's live library, so they stay OUT of git):
`apps/extension/safari-app/build/qa-2026-09-07/` (gitignored) — `1-companion-setup-window.png`
(504×717), `2-safari-popup.png` (3456×1640), `3-safari-settings-extensions.png` (1704×1280); the
same files are in this session's scratchpad. They are evidence, not store assets (§5 has the recipe).
