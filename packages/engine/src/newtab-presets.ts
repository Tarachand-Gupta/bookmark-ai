...
const FAVORITES_HTML = page(
  ...
  [
    ...
    "  var res=await window.call('getFavorites');",
    "  if(!res.ok){out.innerHTML='<p class=\"muted\">Couldn\\u2019t load your data ('+window.__esc(res.error||'unknown error')+').</p>';return;}",
    "  bind(res.data||[]);}",
    "init();",
  ].join("\n"),
);
...
const CONTINUE_HTML = page(
  ...
  [
    ...
    "  if(!res.ok){out.innerHTML='<p class=\"muted\">Couldn\\u2019t load your data ('+window.__esc(res.error||'unknown error')+').</p>';return;}",
    "  if(!res.data||!res.data.enabled){out.innerHTML=",
...
const TIME_SPENT_HTML = page(
  ...
  [
    ...
    "    '<b>Not tracked — on purpose.</b><p>Bookmark AI doesn\\u2019t watch how long you spend on pages, so there\\u2019s nothing to show here. Your saves, sessions, and live tabs power the other templates instead.</p>';return;}",
