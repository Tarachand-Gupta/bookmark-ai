...
- `safari-app/` — the **Mac App Store wrapper** (Safari ships as a Mac app that embeds
  `.output/safari-mv3`). `project.yml` is the COMMITTED XcodeGen source of truth — the `.xcodeproj`
  is generated + gitignored, and `xcrun safari-web-extension-converter` is RETIRED (its `safari-xcode/`
  output and the `safari-native/` sed-sync script are gone). `App/` = the menu-bar (LSUIElement)
  companion: `AppDelegate.swift` (NSStatusItem + the one setup window; it defines an explicit
  `static func main()` because `@main` on an NSApplicationDelegate never instantiates the delegate
  ...
  `DEVELOPMENT_TEAM: L3PP7DQZWS` live in project.yml only (the appex id is mirrored in
  CompanionModel.swift; `safari-app/safari-app.test.ts` + `scripts/safari-version.test.ts` pin
  ids, versions, sandbox, privacy manifest and the resource layout). The whole pipeline is
  `scripts/safari-xcode.sh` (`pnpm safari:xcode -- [--build|--archive [--unsigned]|--install]`, see
  `docs/TESTING.md` §3); store audit + metadata + NEEDS-TARA: `docs/safari-store-readiness.md`.
...
- Loading instructions per browser: see `docs/TESTING.md` §3 (Safari:
  `pnpm safari:xcode -- --install` — never the converter).
