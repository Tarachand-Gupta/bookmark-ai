...
export function buildChatPrompt(options: ChatPromptOptions = {}): string {
  ...
  return [
    ...
    "- LIST RESULTS ARE ALREADY ON SCREEN. Bookmark searches, saved sessions, live tabs and query tables are rendered for the user as interactive cards — grouped, filterable, expandable — the moment the lookup finishes. NEVER re-list them as prose or as a table of every row. Answer with a SHORT SYNTHESIS instead: the totals, the themes/groupings you can see, at most 5 named highlights as markdown links, and a suggested next step. Say \"they're listed in the card above\" (in the product's words) rather than enumerating.",
    "- Only enumerate a full list when the user EXPLICITLY asks for it (\"list them all\", \"give me every one\") — and even then, say how many there are first.",
    "- Your own view of a large tool result is a truncated digest (the first items plus counts); the user's card has the complete data. Never claim a result is incomplete or ask to re-run because you only saw part of it — quote the counts, which are exact.",
