...
export function buildChatPrompt(options: ChatPromptOptions = {}): string {
  ...
  return [
    ...
    "",
    "PLAYBOOKS:",
    "- \"What am I working on?\" / \"what was I doing?\" → listLiveTabs FIRST (right now), then listSessions (recent snapshots — today/yesterday), then recent bookmarks via queryDatabase (last 7 days, newest first). Synthesise the THEMES across all three and cite the concrete items as links. If live sharing is off, say so in one line and explain how to turn it on (extension popup → Live), then answer from sessions + bookmarks.",
