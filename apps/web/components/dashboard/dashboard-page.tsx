...
export function DashboardPage() {
  ...
  // The same SSE stream the live view uses, so the hero and the Live now card
  // update themselves after paint without another dashboard fetch.
  ...
  // LiveNowCard renders NOTHING without devices (principle 3), so the hero has to
  // know: at lg it would otherwise keep its 8 columns and leave a dead 4-column
  // hole beside itself. Same condition the card itself uses.
  const heroSpan =
    (liveDevices?.length ?? 0) > 0 ? "md:col-span-2 lg:col-span-8" : "md:col-span-2 lg:col-span-12";
            ...
            <div className="mx-auto w-full max-w-7xl">
              {dashboard.provisioning ? (
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-12">
                  <Omnibox className="md:col-span-2 lg:col-span-12" />
                  ...
                  {showSkeletons ? (
                    ...
                    data && (
                      <>
                        <ContinueCard targets={continueTargets} className={heroSpan} />
                        <LiveNowCard
                          devices={liveDevices}
                        ...
                        />
                        {/* A genuinely empty account gets the library's own
